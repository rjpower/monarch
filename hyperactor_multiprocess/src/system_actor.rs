...
mod tests {
    ...
    async fn mock_proc_actor(
        ...
        let (local_proc_addr, local_proc_rx) =
            channel::serve(ChannelAddr::any(ChannelTransport::Local))
                .await
                .unwrap();
        // Proc actor ID: world[idx].proc[0].
    ...
    async fn test_join() {
        ...
        ) = mock_proc_actor("test", 0).await; // world name 'test'

        // Join the world.
        system_actor_handle
            .send(SystemMessage::Join {
                proc_id: local_proc_id,                            // test[0]
                world_id,                                          // test
                proc_message_port: local_proc_message_port.bind(), // test[0].proc[0]: `PortHandle<ProcMessage>`
                proc_addr: local_proc_addr,                        // `PortReceiver<ProcMessage>`
                labels: HashMap::new(),
        ...
        // We should be able to send a message to ourselves now, by way of system_proc.
        let (local_u64_port, mut local_u64_receiver) = local_proc_mbox.open_port::<u64>();
        system_proc.post(
            MessageEnvelope::new(
        ...
        assert_eq!(local_u64_receiver.recv().await.unwrap(), 123);
    }
    ...
    #[tokio::test]
    async fn test_supervision_timeout() {
        ...
        let (
            ...
            client_proc_addr,
            client_mailbox,
            client_proc_message_port,
            mut client_proc_message_receiver,
        ) = mock_proc_actor("client", 0).await;

        // Join the world without supervision enabled.
        system_actor_handle
            .send(SystemMessage::Join {
                world_id: world_id.clone(),
                proc_id: client_proc_id.clone(),
                proc_message_port: client_proc_message_port.bind(),
                proc_addr: client_proc_addr,
                labels: HashMap::new(),
                lifecycle_mode: ProcLifecycleMode::ManagedBySystem,
            })
            .unwrap();
        assert_matches!(
            client_proc_message_receiver.recv().await.unwrap(),
            ProcMessage::Joined()
        );

        // Check world status
        let (client_tx, client_rx) = client_mailbox.open_once_port::<SystemSnapshot>();
        system_actor_handle
            .send(SystemMessage::Snapshot {
                filter: SystemSnapshotFilter::all(),
                ret: client_tx.bind(),
            })
            .unwrap();
        ...
        // Use a local proc actor to join the system to trigger supervision expiration.
        let (
            world_id,
            local_proc_id,
            local_proc_addr,
            ...
            local_proc_message_port,
            mut local_proc_message_receiver,
        ) = mock_proc_actor("unreacheable_proc", 1).await;
        // Join the world.
        system_actor_handle
            .send(SystemMessage::Join {
        ...
        // Query an unknow world, should get back None
        let unknown_world_id = id!(unknow_world);
        let (client_supervision_tx, client_supervision_receiver) =
            client_mailbox.open_once_port::<Option<WorldSupervisionState>>();
        let client_supervision_port_ref = client_supervision_tx.bind();
        system_actor_handle
            .send(WorldSupervisionMessage::State(
                unknown_world_id,
                client_supervision_port_ref,
            ))
            .unwrap();
        let msg = client_supervision_receiver.recv().await;
        assert_eq!(msg.unwrap(), None);

        // Wait a bit; supervision should time out.
        RealClock.sleep(2 * timeout).await;

        // Query supervision state,
        let (client_supervision_tx, client_supervision_receiver) =
            client_mailbox.open_once_port::<Option<WorldSupervisionState>>();
        let client_supervision_port_ref = client_supervision_tx.bind();
        ...
        system_actor_handle
            .send(WorldSupervisionMessage::State(
                World::get_real_world_id(&world_id),
                client_supervision_port_ref,
            ))
            .unwrap();

        // Should've gotten the supervision message indicating heartbeat failure
        let msg = client_supervision_receiver.recv().await;
    ...
    async fn test_world_eviction() {
        ...
        // Join the world without being managed by the system.
        system_actor_handle
            .send(SystemMessage::Join {
                proc_id: client_proc_id.clone(),
                world_id: world_id.clone(),
                ...
                proc_addr: client_proc_addr,
                labels: HashMap::new(),
                lifecycle_mode: ProcLifecycleMode::Detached,
            })
            .unwrap();
        assert_matches!(
            client_proc_message_receiver.recv().await.unwrap(),
        ...
        let snapshot = system_actor_handle
            .snapshot(&client_mailbox, SystemSnapshotFilter::all())
            .await
            .unwrap();
        ...
        // The world will become unhealthy first
        RealClock.sleep(2 * timeout).await;
        ...
        let mut state = system_actor_handle
            ...
            .unwrap();
        while iter < 100 {
            if state.procs.values().any(|p| !p.is_healthy()) {
                break;
            }
            iter += 1;
        ...
        assert!(state.procs.values().any(|p| !p.is_healthy()));
        // The snapshot isn't updated yet, so we still see 1 world.
        let mut snapshot = system_actor_handle
            .snapshot(&client_mailbox, SystemSnapshotFilter::all())
            .await
            .unwrap();
        assert!(snapshot.worlds.len() == 1);

        // Stopping the world will evict it.
        let (client_tx, client_rx) = client_mailbox.open_once_port::<()>();
        ...
        // Wait for the stop to timeout and the world to be evicted.
        RealClock.sleep(10 * timeout).await;

        // The world will be evicted if it has not been.
        snapshot = system_actor_handle
            .snapshot(&client_mailbox, SystemSnapshotFilter::all())
            .await
            .unwrap();

        assert_eq!(snapshot.worlds.len(), 0, "{:?}", snapshot);
    }

    #[tokio::test]
    async fn test_proc_managing_system() {
        // Use small durations to fail fast.
        let timeout: Duration = Duration::from_secs(2);
...

        {
            ...
            assert_eq!(*received, ActorStatus::Idle);
        }
        ...
        system_actor_handle
            .send(SystemMessage::Join {
                ...
                proc_addr: local_proc_addr,
                labels: HashMap::new(),
                lifecycle_mode: ProcLifecycleMode::ManagingSystem,
            })
            .unwrap();
        ...
        let _ = system_actor_handle
            ...
            .await;
        client_rx.recv().await.unwrap();
        // Wait for the stop to timeout and the system should be stopped
        RealClock.sleep(10 * timeout).await;
        {
            assert!(sys_status_rx.borrow().has_changed());
            let received = sys_status_rx.borrow_and_update();
