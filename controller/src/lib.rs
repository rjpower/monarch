...
mod tests {
    ...
    async fn test_sim_supervision_failure() {
        ...
        let system_sim_addr = ChannelAddr::Sim(
            SimAddr::new(
                system_addr.clone(),
                proxy_addr.clone(),
                system_addr.clone(),
        ...
        let server_handle = System::serve(
            system_sim_addr,
            SystemActorParams::new(Duration::from_secs(1000), Duration::from_secs(1000)),
        )
        .await
        .unwrap();
        ...
        let controller_id = id!(controller[0].root);
        let proc_id = id!(world[0]);
        let controller_proc_addr = "local!2".parse::<ChannelAddr>().unwrap();
        let controller_proc_sim_addr = SimAddr::new(
            controller_proc_addr.clone(),
            proxy_addr.clone(),
            system_addr.clone(),
            proxy_addr.clone(),
        )
        .unwrap();
        let controller_proc_bootstrap_addr = ChannelAddr::Sim(controller_proc_sim_addr.clone());
        let controller_proc_listen_addr = ChannelAddr::Sim(controller_proc_sim_addr.clone());

        let (_, actor_ref) = ControllerActor::bootstrap(
            controller_id.clone(),
            controller_proc_listen_addr,
            controller_proc_bootstrap_addr,
            ControllerParams {
                world_size: 1,
                comm_actor_ref: ActorRef::attest(controller_id.proc_id().actor_id("comm", 0)),
                worker_gang_ref: GangId(
    ...
    async fn test_supervision_failure() {
        ...
        let server_handle = System::serve(
            ChannelAddr::any(ChannelTransport::Local),
            SystemActorParams::new(timeout.clone(), timeout.clone()),
        )
        .await
        .unwrap();
