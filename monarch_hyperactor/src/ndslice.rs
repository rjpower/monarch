...
#[pyclass(name = "Slice", frozen, module = "monarch._monarch.ndslice")]
#[derive(Clone)]
pub struct PySlice {
...
pub fn init_pymodule(module: &Bound<'_, PyModule>) -> PyResult<()> {
    let ndslice_mod = PyModule::new_bound(module.py(), "ndslice")?;
    ndslice_mod.add_class::<PySlice>()?;
    module.add_submodule(&ndslice_mod)?;
    Ok(())
}
