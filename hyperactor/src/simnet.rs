            ...
            tokio::spawn(async move {
                'outer: loop {
                    ...
                    if let Ok(Ok(msg)) = timeout(Duration::from_millis(100), rx.recv()).await {
                        ...
                        let event: Box<dyn Event> = match &proxy_message.addr {
                            Some(sim_addr) => Box::new(MessageDeliveryEvent::new(
                                sim_addr.clone(),
                                proxy_message.data,
                            )),
                            None => {
...
impl SimNet {
    ...
    async fn run(&mut self, mut event_rx: UnboundedReceiver<Box<dyn Event>>) {
        'outer: loop {
