...
pub struct SimNetHandle {
    ...
    operational_message_tx: Sender<OperationalMessage>,
    /// A receiver to receive simulator operational messages.
    /// The receiver can be moved out of the simnet handle.
    operational_message_rx: Arc<Mutex<Option<Receiver<OperationalMessage>>>>,
}

impl SimNetHandle {
    /// Sends an event to be scheduled onto the simnet's event loop
    pub async fn send_event(&self, event: Box<dyn Event>) -> Result<(), SimNetError> {
    ...
    pub async fn bind(&self, address: ChannelAddr) -> Result<(), SimNetError> {
        self.pending_event_count
            .fetch_add(1, std::sync::atomic::Ordering::SeqCst);
        self.event_tx
    ...
    pub async fn operational_message_receiver(
        &self,
    ) -> Result<Receiver<OperationalMessage>, SimNetError> {
...
impl ProxyMessage {
    ...
    pub fn new(addr: Option<SimAddr>, data: Serialized) -> Self {
        Self { addr, data }
    }
}
...
impl ProxyHandle {
    ...
    async fn start(
        ...
        event_tx: Sender<Box<dyn Event>>,
        pending_event_count: Arc<AtomicUsize>,
        to_event: Option<fn(SimAddr, Serialized) -> Box<dyn Event>>,
        operational_message_tx: Sender<OperationalMessage>,
    ) -> anyhow::Result<Self> {
        ...
        /// Default event generation function that will use the default SENDER for sim channel.
        fn to_sim_msg(addr: SimAddr, data: Serialized) -> Box<dyn Event> {
            Box::new(MessageDeliveryEvent::new(addr, data))
        }
        let to_event = to_event.unwrap_or(to_sim_msg);
        let join_handle = {
            ...
            tokio::spawn(async move {
                'outer: loop {
                    ...
                    if let Ok(Ok(msg)) = timeout(Duration::from_millis(100), rx.recv()).await {
                        ...
                        let event = match &proxy_message.addr {
                            Some(sim_addr) => to_event(sim_addr.clone(), proxy_message.data),
                            None => {
...
pub fn start(private_addr: ChannelAddr, max_duration_ms: u64) -> anyhow::Result<SimNetHandle> {
    ...
    let (event_tx, event_rx) = mpsc::channel::<Box<dyn Event>>(100);
...
impl SimNet {
    ...
    async fn schedule_event(&mut self, mut event: Box<dyn Event>) {
        ...
        if let Some(records) = &self.records {
            records.lock().await.push(SimulatorEventRecord {
                ...
                end_at: scheduled_time_ms,
            });
        }
        self.state
            .scheduled_events
            .entry(scheduled_time_ms)
    ...
    async fn run(&mut self, mut event_rx: Receiver<Box<dyn Event>>) {
        'outer: loop {
            ...
            match tokio::time::timeout(Duration::from_millis(10), event_rx.recv()).await {
                Ok(Some(message)) => {
                    self.schedule_event(message).await;
                    continue;
                }
                _ => {}
            }
            ...
            {
                let no_scheduled_events = self.state.scheduled_events.is_empty();
...
mod tests {
    ...
    impl MessageDeliveryEvent {
        fn new(dest_addr: SimAddr, data: Serialized, dispatcher: Option<TestDispatcher>) -> Self {
            Self {
                dest_addr,
                data,
                duration_ms: 1,
    ...
    async fn test_simnet_config() {
        ...
        nw_handle.send_event(msg).await.unwrap();
    ...
    async fn test_simnet_debounce() {
        ...
        let latency = Duration::from_millis(100000);
        nw_handle
    ...
    async fn test_sim_dispatch() {
        ...
        nw_handle.send_event(one).await.unwrap();
        nw_handle.send_event(two).await.unwrap();
        nw_handle.send_event(three).await.unwrap();
