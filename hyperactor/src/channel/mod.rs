...
use crate::channel::sim::OverlayEdgeConfig;
...
pub enum ChannelTransport {
    ...
    /// Sim is a simulated channel for testing. Its transport is a tuple of
    /// src_proxy address and dst_proxy address.
    Sim(OverlayEdgeConfig),
...
impl ChannelAddr {
    ...
    pub fn any(transport: ChannelTransport) -> Self {
        match transport {
            ...
            ChannelTransport::Sim(overlay_edge_config) => sim::any(overlay_edge_config),
    ...
    pub fn transport(&self) -> ChannelTransport {
        match self {
            ...
            Self::Sim(sim_addr) => ChannelTransport::Sim(OverlayEdgeConfig::new(sim_addr.clone())),
...
mod tests {
    ...
    fn test_channel_addr() {
        ...
        for (raw, parsed) in cases_ok.iter().zip(src_ok.clone()).map(|(a, b)| {
            let proxy_a_str = "unix!@proxy_a";
            let proxy_b_str = "unix!@proxy_b";
            (
                format!("sim!{},{},{},{}", a.0, &proxy_a_str, b.0, &proxy_b_str),
                ChannelAddr::Sim(
                    SimAddr::new(
                        a.1.clone(),
                        proxy_a_str.parse().unwrap(),
                        b.1.clone(),
                        proxy_b_str.parse().unwrap(),
                    )
                    .unwrap(),
                ),
            )
