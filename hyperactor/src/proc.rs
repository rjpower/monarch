...
impl<A: Actor> Instance<A> {
    ...
    async fn serve(mut self, mut actor: A) {
        ...
        let _ = self.status_tx.send(actor_status);
    }
    ...
    async fn run_actor_tree(&mut self, actor: &mut A) -> Result<(), ActorError> {
        ...
        let _ = self.status_tx.send(ActorStatus::Stopping);
    ...
    #[crate::instrument(name = "actor_receive_loop", fields(actor_id=tracing::field::display(self.self_id())))]
    async fn run(&mut self, actor: &mut A) -> Result<(), ActorError> {
        ...
        let _ = self.status_tx.send(ActorStatus::Initializing);
        actor
            .init(self)
            .await
        ...
        let need_drain;
        'messages: loop {
            let _ = self.status_tx.send(ActorStatus::Idle);
            tokio::select! {
                work = self.work_rx.recv() => {
                    MESSAGES_RECEIVED.add(1, hyperactor_telemetry::kv_pairs!("actor_id" => self.self_id().to_string()));
