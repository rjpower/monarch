"""
This contains a simple FSDP implementation based on parametrization for the
parameter all-gather and gradient generator for the gradient reduce-scatter.
...
class FSDPCtx:
    ...
    def deactivate_ctx():
        """
        This can be used if the user wants to access ``module.param`` as the
        sharded parameter instead of the unsharded parameter.
        """
        prev_is_active = FSDPCtx.is_active
        FSDPCtx.is_active = False
        try:
            yield
        finally:
            FSDPCtx.is_active = prev_is_active
...
# Similar to a parametrization
class FSDPUnshard(nn.Module):
    def __init__(self, fsdp_params: List[FSDPParam], dim: str):
        super().__init__()
        self.fsdp_params = fsdp_params
        self.dim = dim

    def unshard(self, fsdp_param: FSDPParam, sharded_param: Tensor) -> Tensor:
        assert fsdp_param.sharded_param is sharded_param, "assume for now"
        # Under this design, FSDP all-gather runs outside of autograd, and we
        # rely on the gradient generator to insert FSDP reduce-scatter
        if fsdp_param.unsharded_param is not None:
            fsdp_param.unsharded_param.requires_grad_(sharded_param.requires_grad)
            return fsdp_param.unsharded_param
        with torch.no_grad():
            unsharded_param = unshard_tensor(sharded_param, self.dim)
        unsharded_param.requires_grad_(sharded_param.requires_grad)
        fsdp_param.unsharded_param = unsharded_param
        return unsharded_param

    def forward(self, fsdp_param: FSDPParam, sharded_param: Tensor) -> Tensor:
        if FSDPCtx.is_active:
            return self.unshard(fsdp_param, sharded_param)
        return sharded_param


def register_fsdp_unshard(module: nn.Module, fsdp_params, dim) -> None:
    fsdp_unshard = FSDPUnshard(fsdp_params, dim)

    def param_fn(self, param_name: str):
        assert param_name in self._parameters
        assert param_name in self._param_name_to_fsdp_param
        fsdp_param = self._param_name_to_fsdp_param[param_name]
        sharded_param = self._parameters[param_name]
        return fsdp_unshard(fsdp_param, sharded_param)

    param_names = [fsdp_param.param_name for fsdp_param in fsdp_params]
    param_name_to_property = {
        param_name: property(functools.partial(param_fn, param_name=param_name))
    ...
    # Do not go through `nn.Module.__setattr__` to avoid it being registered as
    # a submodule
    object.__setattr__(module, "_fsdp_unshard", fsdp_unshard)
...
def apply_fsdp(
    ...
    *,
    ...
    """
    Applies FSDP to the parameters in ``module``, sharding them across
    ``device_mesh`` ``dim``.

    The design separates the all-gather (AG) and reduce-scatter (RS) support.
    - For AG, the base logic emulates parametrization[0] such that the
    parameters are changed to properties that run per-parameter AG, and
    batched AGs can be done via explicit prefetching.
    - For RS, the logic requires a custom call to backward that uses the
    gradient generator to allow (batched) RSs to be called between gradient
    computation.

    The current design does not support resharding after forward, where
    parameters are freed after forward and re-all-gathered in backward.

    [0]: https://pytorch.org/tutorials/intermediate/parametrizations.html
    """
    root_module = module
    for module in root_module.modules():
        ...
        for param_name, param in module.named_parameters(recurse=False):
            fsdp_param = FSDPParam(param, param_name, device_mesh, dim, device)
            module._param_name_to_fsdp_param[param_name] = fsdp_param
            module.register_parameter(param_name, fsdp_param.sharded_param)
        if module._param_name_to_fsdp_param:
            fsdp_params = list(module._param_name_to_fsdp_param.values())
            register_fsdp_unshard(module, fsdp_params, dim)
    return root_module
...
def register_forward_prefetch_for_modules(
    ...
    def prefetch_hook(self, *args):
        handle = batched_all_gather(
            ...
            comm_stream,
            dtype,
            # target_sharded_params, device_mesh, dim, None, dtype
        )
    ...
    def wait_hook(self, *args):
        if (handle := getattr(self, "_fsdp_all_gather_handle", None)) is not None:
            ...
            for unsharded_tensor, fsdp_param in zip(
                ...
                # NOTE: This is a plain `Tensor`, not `nn.Parameter`.
                fsdp_param.unsharded_param = unsharded_tensor
...
check_equal = remote(
    "examples.fsdp.fsdp_parametrization.check_equal_impl", propagate=lambda ts: False
)
...
def assert_equal_on_coordinates(ts: Tuple[Tensor, Tensor], coordinates):
    for coordinate in coordinates:
        are_equal = (
            remote(
                "examples.fsdp.fsdp_parametrization.check_equal_impl",
            )
