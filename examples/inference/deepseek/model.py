...
def get_model_parallel_rank() -> int:
    """Get the rank of the model parallel GPU."""
    from monarch import get_active_mesh

    device_mesh = get_active_mesh()
    return device_mesh.rank("gpu")


def get_model_parallel_world_size() -> int:
    """Return world size for model parallel group."""
    from monarch import get_active_mesh

    device_mesh = get_active_mesh()
    return device_mesh.size("gpu")
...
class Expert(nn.Module):
    ...
    def __init__(self, dim: int, inter_dim: int, local_idx: int):
        """
...
class MoE(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.gate = Gate(args).cuda()
        self.experts = nn.ModuleList(
            [
                Expert(args.dim, args.moe_inter_dim, i)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for expert in self.experts:
            i = expert.local_idx + rank * self.n_local_experts
            if counts[i] == 0:
                continue
...
class Block(nn.Module):
    ...
    def __init__(self, layer_id: int, args: ModelArgs):
        ...
        self.ffn = (
            ...
            else OpaqueModule("deepseek.model.MoE", args)
        )
        self.is_moe = layer_id >= args.n_dense_layers
    ...
    def forward(
        ...
        h = self.ffn_norm(x)
        if self.is_moe:
            h = self.ffn(h, propagate=lambda self, x: x.clone())
        else:
            h = self.ffn(h)
        x = x + h
        return x
...
class Transformer(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, start_pos: int = 0):
        ...
        logits = self.head(h)
        if world_size > 1:
            logits = logits.reduce("gpu", "stack")
        return logits
