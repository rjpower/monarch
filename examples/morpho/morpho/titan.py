# shim to launch torchtitan code for comparison and testing purposes

# pyre-unsafe

# as we support more config options in this repo, we will
# pass them to torchtitan by translating them into torchtitans config language
...
from morpho.subprocess import call_configured
...
def train_from_config(job_config: Any, debug: DebugConfig):
    init_logger()
    ...
    with maybe_enable_profiling(
        ...
        job_config, global_step=train_state.step
    ) as memory_profiler, inject_debug_behavior(debug, new_random_context) as (
        observer,
        debug_filename,
    ):
        if observer is not None:
            for layer in model.layers.values():
                layer.register_forward_hook(
                    lambda module, args, output: observer.observe("layer", output)
                )
        while train_state.step < job_config.training.steps:
            ...
            if parallel_dims.pp_enabled:
                ...
                loss = (
                    torch.mean(torch.stack(losses))  # type: ignore
                    if is_last_stage
            ...
            if observer is not None:
                for p in reversed([p for m in model_parts for p in m.parameters()]):
                    observer.observe("grad", p.grad)
            ...
            if (
                ...
                if (
                    ...
                    global_avg_loss, global_max_loss = (
                        utils.dist_mean(avg_loss, world_mesh["dp_cp"]),  # type: ignore
...
def train_from_args(args, debug: DebugConfig):
    config = JobConfig()
    config.parse_args(args)
    result = train_from_config(config, debug)
...
def new_random_context(seed: int) -> RandomContext:
    def _get_state():
        return (
            torch.get_rng_state(),
            torch.cuda.get_rng_state(int(os.environ["LOCAL_RANK"])),
        )

    def _set_state(state):
        cpu, device = state
        torch.set_rng_state(cpu)
        torch.cuda.set_rng_state(device, int(os.environ["LOCAL_RANK"]))

    orig = _get_state()
    torch.manual_seed(seed)
    mine = _get_state()
    _set_state(orig)
    return RandomContext(mine, _get_state, _set_state)
...
def train(
    ...
    with tempfile.NamedTemporaryFile("w") as temp:
        ...
        args = [
            ...
            morpho.debug.data.test_tokenizer,
            "--training.dataset_path",
            morpho.debug.data.test_dataset,
            "--training.data_parallel_replicate_degree",
            str(machine.ngpu),
        ]
        if training.deterministic:
            args.append("--training.deterministic")
        calls = []
        for i in range(machine.ngpu):
            rank = str(i)
            size = str(machine.ngpu)
            environment = {
            ...
            if machine.visible_gpus is not None:
                environment["CUDA_VISIBLE_DEVICES"] = machine.visible_gpus
            calls.append(call_configured(env=environment)(train_from_args, args, debug))
