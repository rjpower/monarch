...
///
/// This wrapper processes the input buffer by searching for newlines and inserting
/// the rank prefix at the beginning of each line. It has minimal overhead with no
/// internal buffering and avoids allocations during normal operation (except for
/// the one-time prefix allocation during construction).
    ...
    W: io::AsyncWrite + Unpin,
{
    fn poll_write(
        ...
        while pos < buf.len() {
            // If we need to write a prefix, do it first
            if this.need_prefix {
                match Pin::new(&mut this.inner).poll_write(cx, &this.prefix) {
                    Poll::Ready(Ok(n)) if n == this.prefix.len() => {
                        this.need_prefix = false;
                    }
                    Poll::Ready(Ok(_)) => {
                        // Partial write of prefix - this is an error condition
                        return Poll::Ready(Err(io::Error::new(
                            io::ErrorKind::WriteZero,
                            "Partial write of line prefix - unable to recover",
                        )));
                    }
            ...
            // Now write data until the next newline or end of buffer
            let remaining = &buf[pos..];
            if let Some(newline_offset) = remaining.iter().position(|&b| b == b'\n') {
                // Write up to and including the newline
                let end_pos = pos + newline_offset + 1;
                let to_write = &buf[pos..end_pos];

                match Pin::new(&mut this.inner).poll_write(cx, to_write) {
                    Poll::Ready(Ok(n)) if n == to_write.len() => {
                        total_written += n;
                        pos = end_pos;
                        this.need_prefix = true; // Next write will need a prefix
                    }
                    Poll::Ready(Ok(n)) => {
                        // Partial write - return progress so far
                        total_written += n;
                        return Poll::Ready(Ok(total_written));
                    }
                    Poll::Ready(Err(e)) => return Poll::Ready(Err(e)),
...
mod tests {
    ...
    async fn test_line_prefixing_writer_multiple_writes() {
        ...
        // Check that prefix was added to each line
        let written_data = data.lock().unwrap();
        assert_eq!(*written_data, b"[42] First line\n[42] Second line\n");
    }
    ...
    #[tokio::test]
    async fn test_line_prefixing_writer_pending_after_progress() {
        // Test case: we make some progress then hit pending
        let (mock_writer, data) = PartialMockWriter::new(vec![
            WriteResult::Full,    // Prefix writes fully
            WriteResult::Full,    // "First line\n" writes fully
            WriteResult::Full,    // Next prefix writes fully
            WriteResult::Pending, // But "Second line" hits pending
        ]);
        let mut writer = LinePrefixingWriter::new(42, mock_writer);

        // Write two lines - should get partial success
        let result = std::future::poll_fn(|cx| {
            use tokio::io::AsyncWrite;
            Pin::new(&mut writer).poll_write(cx, b"First line\nSecond line")
        })
        .await;

        // Should return bytes written for first complete line
        assert_eq!(result.unwrap(), 11); // "First line\n"
